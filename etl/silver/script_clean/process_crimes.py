...
def process_crimes():
    """
    Traite les crimes et délits 2019 & 2024.

    Méthode : sum(nombre de faits) par département/année, divisé par la
    population INSEE du fichier × 1000, pour obtenir un taux pour mille.

    Gestion du double-comptage stupéfiants :
    - En 2019 : "Usage de stupéfiants" = "(AFD)" + "(hors AFD)" → exclure les sous-catégories
    - En 2024 : les trois indicateurs sont indépendants → exclure le parent "Usage de stupéfiants"
    """
    ...
    df = df[df['annee'].isin([2016, 2019, 2024])].copy()
    print(f"Filtered to {len(df)} rows for years 2016, 2019 and 2024")

    # Pour les 3 années : "Usage de stupéfiants" = "(AFD)" + "(hors AFD)"
    # → garder le parent, exclure les sous-catégories (identique pour 2016, 2019 et 2024)
    mask_excl = df['indicateur'].isin([
        'Usage de stupéfiants (AFD)', 'Usage de stupéfiants (hors AFD)'
    ])
    df = df[~mask_excl].copy()
    print(f"After double-count fix: {len(df)} rows ({df['indicateur'].nunique()} indicateurs/année)")

    df_agg = df.groupby(['Code_departement', 'annee'], as_index=False).agg(
        total_faits=('nombre', 'sum'),
        insee_pop=('insee_pop', 'first'),
    )

    df_agg['taux_pour_mille'] = (df_agg['total_faits'] / df_agg['insee_pop'] * 1000).round(2)
    df_agg = df_agg.rename(columns={'Code_departement': 'code_departement'})
    df_agg = df_agg[['code_departement', 'annee', 'taux_pour_mille']]
